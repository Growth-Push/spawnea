...
- **[Local Control API and MCP](control-api-mcp.md)**: Local Unix-domain socket MCP server, root/bootstrap scoping, session creation, prompt/turn lifecycle, and protocol contract. The parent-session bootstrap sections are explicitly marked as target behavior where implementation is pending.
